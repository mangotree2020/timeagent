...
import { MinuteWheel, TimeWheelPicker } from '@/components/wheel-picker';
...
function RoutineForm({ draft, onChange, dailySkip, onRestoreDaily }: DraftFormProps & { dailySkip: OncePerDaySkip | null; onRestoreDaily: (entry: OncePerDaySkip['removed'][number]) => void }) {
  ...
  const setMinutes = (id: string, minutes: number) => {
    ...
    onChange({ routines: draft.routines.map((routine) => routine.id === id
      ? { ...routine, minutes: Math.max(1, Math.round(minutes)), minutesEditedByUser: true }
      : routine) });
  };
  ...
  return <View style={styles.form}><SectionTitle>무엇을 준비해야 하나요?</SectionTitle><Text style={type.bodyMuted}>설정한 기본 추천과 최근 기록을 시작점으로 보여드려요. 나에게 맞게 항목과 시간을 조정할 수 있어요.</Text>
    ...
    {draft.routines.map((item) => <Card key={item.id} style={styles.routine}><View style={styles.routineTop}><View style={styles.routineIcon}><AppIcon name={iconForRoutine(item.id, item.icon)} size={22} /></View><View style={styles.routineCopy}><Text style={[type.body, { fontWeight: '800' }]}>{item.label}</Text>{item.minutesEditedByUser ? <Text style={type.caption}>내가 정한 시간</Text> : personalization?.routineMinutes[item.id] ? <Text style={type.caption}>최근 평균 {personalization.routineMinutes[item.id].minutes}분 적용 중</Text> : null}</View><Pressable accessibilityRole="button" accessibilityLabel={`${item.label} 준비 행동 삭제`} accessibilityState={{ disabled: onlyOneLeft }} disabled={onlyOneLeft} onPress={() => removeStep(item.id)} style={({ pressed }) => [styles.removeRoutine, onlyOneLeft && styles.removeRoutineDisabled, pressed && styles.pressedRow]}><AppIcon name="trash" size={18} iconColor={onlyOneLeft ? undefined : '#C2413A'} /></Pressable></View><View style={styles.routineWheel}><Text style={type.caption}>준비 시간</Text><MinuteWheel label={`${item.label} 시간`} value={effectiveRoutineMinutes(item, personalization)} onChange={(minutes) => setMinutes(item.id, minutes)} testID={`routine-minutes-${item.id}`} /></View></Card>)}
...
const createStyles = (c: AppPalette) => StyleSheet.create({
  ...
  routine: { padding: space.lg, gap: space.sm }, routineTop: { flexDirection: 'row', alignItems: 'center', gap: space.sm }, routineWheel: { flexDirection: 'row', alignItems: 'center', gap: space.md }, routineIcon: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: c.surfaceMuted }, addRoutine: { minHeight: 48, flexDirection: 'row', gap: space.sm, borderWidth: 1, borderStyle: 'dashed', borderColor: c.cyan, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center' }, addRoutineText: { color: c.deepBlue, fontWeight: '800' },

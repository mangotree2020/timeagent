...
function WeatherDetail({ status, weather, onRetry }: { status: WeatherDetailStatus; weather: WeatherSnapshot | null; onRetry: () => void }) {
  if (status === 'checking') {
    return <Card style={styles.state}><ActivityIndicator color={color.deepBlue} /><Text style={type.heading}>현재 위치 날씨를 확인하고 있어요</Text><Text style={type.bodyMuted}>잠시만 기다려 주세요.</Text></Card>;
  }
  ...
  return (
    <>
      ...
      >
        ...
        <AppIcon name="chevronRight" size={15} iconColor={color.textMuted} />
      </Pressable>
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  stateIcon: { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', backgroundColor: color.ice },
  currentCard: { gap: space.lg, borderColor: 'transparent' },
  currentTop: { flexDirection: 'row', alignItems: 'center', gap: space.md },
  weatherIcon: { width: 62, height: 62, borderRadius: 31, alignItems: 'center', justifyContent: 'center', backgroundColor: color.ice },
  currentCopy: { flex: 1, gap: 2 },
  condition: { color: color.navy, fontSize: 20, lineHeight: 27, fontWeight: '900' },
  freshness: { color: color.textMuted, fontSize: 13, lineHeight: 18 },
  temperature: { color: color.deepBlue, fontSize: 44, lineHeight: 50, fontWeight: '900', letterSpacing: -1.5 },
  advice: { padding: space.lg, overflow: 'hidden', borderRadius: radius.md, backgroundColor: '#E4EFFF', color: color.navy, fontSize: 16, lineHeight: 23, fontWeight: '800' },
  metricRow: { flexDirection: 'row', gap: space.sm },
  metric: { flex: 1, minHeight: 90, alignItems: 'center', justifyContent: 'center', gap: 3, borderRadius: radius.lg, backgroundColor: color.surface },
  metricLabel: { color: color.textMuted, fontSize: 13, lineHeight: 18, fontWeight: '700' },
  metricValue: { color: color.navy, fontSize: 22, lineHeight: 29, fontWeight: '900' },
  rainCard: { flexDirection: 'row', alignItems: 'center', gap: space.md, borderColor: 'transparent' },
  rainIcon: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: color.ice },
  rainCopy: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  rainLabel: { color: color.navy, fontSize: 15, lineHeight: 21, fontWeight: '800' },
  rainValue: { color: color.deepBlue, fontSize: 20, lineHeight: 27, fontWeight: '900' },
  source: { minHeight: 44, alignSelf: 'center', flexDirection: 'row', alignItems: 'center', gap: 2, paddingHorizontal: space.md },
  sourceText: { color: color.textMuted, fontSize: 12, lineHeight: 17, fontWeight: '700', textDecorationLine: 'underline' },

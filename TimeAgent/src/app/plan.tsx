...
export default function PlanScreen() {
  ...
  return (
    <Screen>
      ...
      {mapOpen ? (schedule.destinationCoordinate ? <DestinationMap coordinate={schedule.destinationCoordinate} route={route} onExpand={() => setMapExpanded(true)} /> : <View style={styles.mapUnavailable}><AppIcon name="location" size={26} iconColor={c.textMuted} /><Text style={type.bodyMuted}>저장된 지도 좌표가 없습니다. 약속 수정에서 장소를 다시 선택해 주세요.</Text></View>) : null}
      {schedule.destinationCoordinate ? <Modal visible={mapExpanded} animationType="slide" onRequestClose={() => setMapExpanded(false)}>
        <SafeAreaView edges={['top', 'right', 'bottom', 'left']} style={styles.fullMapPage}>
          <View style={styles.fullMapHeader}>
            <View style={styles.flex}><Text style={styles.fullMapTitle}>{route ? '도착 장소와 이동 경로' : '도착 장소 지도'}</Text><Text style={type.bodyMuted} numberOfLines={1}>{schedule.destinationAddress || schedule.destination}</Text></View>
            <Pressable accessibilityRole="button" accessibilityLabel="지도 전체 화면 닫기" onPress={() => setMapExpanded(false)} style={({ pressed }) => [styles.roundButton, pressed && styles.confirmedPressed]}><AppIcon name="close" size={20} iconColor={c.deepBlue} /></Pressable>
          </View>
          <View style={styles.fullMapBody}><DestinationMap fill coordinate={schedule.destinationCoordinate} route={route} /></View>
        </SafeAreaView>
      </Modal> : null}
      ...
      <View style={styles.actions}>
        ...
        {isPending ? <View style={styles.secondary}><View style={styles.flex}><Button label="준비 시간 수정" variant="secondary" onPress={editPreparation} /></View><View style={styles.flex}><Button label="플랜 B 보기" variant="secondary" onPress={() => router.push('/plan-b')} /></View></View> : null}
        {!isPending && activeConfirmedPlanId ? <><View style={styles.secondary}><View style={styles.flex}><Button label="준비 시간 수정" variant="secondary" onPress={editPreparation} /></View><View style={styles.flex}><Button label="플랜 B 보기" variant="secondary" onPress={() => router.push('/plan-b')} /></View></View><View style={styles.manageActions}><View style={{ flex: 1 }}><Button label="약속 수정" variant="secondary" onPress={editAppointment} /></View><View style={{ flex: 1 }}><Button label="약속 삭제" variant="dangerGhost" onPress={() => setDeleteOpen(true)} /></View></View></> : null}

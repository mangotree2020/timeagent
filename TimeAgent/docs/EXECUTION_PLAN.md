...
## 2026-08-20 다국어 표시는 Phase 2로 분리 (결정)

- Observe: 설정에 언어 선택 버튼을 넣으면서 "지원 언어는 한국어·영어·일본어·중국어"로 정해졌다. 선택 UI와 실제 번역은 크기가 전혀 다른 일이다.
- Measure: 한글 문구가 들어간 소스 파일 79개, 문자열 약 1,950개(따옴표 1,750 · 템플릿 196)다. 3개 언어면 번역문만 약 5,800개이고, 여기에 모든 호출부를 `t()` 경유로 바꾸는 리팩터링이 붙는다. 알림 문구와 TTS 음성, 서버 프롬프트가 한국어 기준인 AI 어시스턴트 응답까지 언어를 따라가야 하는지는 아직 정하지 않았다.
- Decide: 이번 회차에는 선택 UI와 저장까지만 넣고 번역은 Phase 2로 분리한다. 절반만 번역된 상태로 배포하면 화면마다 언어가 섞여, 번역이 없는 지금보다 나쁘다.
- Implement: `app-settings` v5에 `language`를 두고 기본값과 기존 설정 마이그레이션을 모두 `ko`로 고정했다. 설정 화면 버튼 아래에 `화면 문구는 아직 한국어로 표시되며, 선택은 이 기기에 저장됩니다.`를 붙여 지금 무엇이 되고 무엇이 안 되는지 밝힌다.
- Next: Phase 2에서 i18n 레이어 도입 → 화면 문구 이관 → 알림·TTS·어시스턴트 언어 정책 결정 순으로 진행한다.
...
## 2026-08-21 준비 단계 알람·1분 전 예고·자동 진행 화면·음성 이동수단 후속 안내 실기기 검증 (완료)

- Accept: 준비 단계가 끝나면 사용자가 끌 때까지 알람이 울리고, 끄는 행동이 곧 단계 완료다. 2분 이상 단계는 마지막 1분이 시작될 때 `1분 남았습니다.`를 말한다. 준비 시작 시각이 되어 세션이 스스로 시작되면 진행 화면이 저절로 열린다. 음성 화면에서 이동수단을 탭으로 고른 뒤에도 남은 질문이나 지도 확인 안내가 음성으로 이어진다.
- Implement: `progress.tsx`에 `expo-audio` 반복 재생 알람(`assets/sounds/step-alarm.wav`)과 1분 전 TTS를 추가하고 완료 버튼 문구를 `알람 끄고 … 완료`로 바꿨다. `notification-service.ts`에 importance MAX·진동·`USAGE_ALARM` 채널 `on-time-alarm`을 분리해 `step-end`·`prep-start`만 그 채널로 보내고, `one-minute-left` 알림 종류를 추가했으며 저장된 예약 종류 검증을 전 종류로 넓혔다. `schedule-context.tsx`는 `source === 'auto'` 시작 시 `/progress`로 이동하고, `step-end` 알림 본문 탭을 `PROGRESS_ADVANCE_ACTION`으로 해석한다. `voice-schedule-assistant.ts`의 `voiceGuidanceAfterTransportChoice`가 이동수단 탭 뒤 후속 안내를 정한다.
- Verify: `npm run verify` TypeScript·ESLint·Jest 479/479 통과. 업로드 키 서명 arm64 release APK(735 tasks, 11분)를 Samsung Android 12 `SM-N971N`에 데이터 유지 방식으로 덮어 설치했다(1.0.9·versionCode 9, `READ_CALENDAR`만 포함·`WRITE_CALENDAR` 없음, SHA-256 `77c7b2f2a510bc68af68869753bf62b37269c7b3d60a30d53e1f981ffbeb8206`). 실기기에서 샤워 2분·옷 입기 2분·짐 챙기기 5분 계획을 확정하자 준비 시작 시각이 이미 지나 세션이 자동 시작되며 진행 화면이 스스로 열렸다. dumpsys audio 기준으로 단계 시작 코치 TTS 17:23:21, 1분 전 TTS 17:24:19와 17:26:25(각 단계 종료 정확히 60초 전), 앱 알람 플레이어 시작 17:25:20·17:27:26(각 단계 종료 1초 뒤)이 기록됐고, 알람은 사용자가 `알람 끄고 … 완료`를 누른 17:25:25·17:27:56에 멈추며 다음 단계와 코치 TTS로 넘어갔다. 알림 채널은 `on-time-alarm`(importance 5, 진동 [0,400,200,400,200,400], USAGE_ALARM)과 `on-time-schedule`(importance 4)로 분리 생성됐고 `준비를 시작할 시간이에요`·`1분 남았습니다.`·`짐 챙기기 예정 시간이 끝났어요`·`이제 출발할 시간이에요` 알림이 올바른 채널로 게시됐다. 음성 화면에서는 Mac TTS 발화 `내일 오후 세시에 서면역에서 치과 예약`이 한 턴으로 인식되어 이동수단 선택지가 나왔고, `지하철`을 탭하자 같은 초에 후속 안내 TTS가 재생되며 상태가 `대화 대기`로 유지됐다(저장하지 않고 닫음). 치명적 Android·React Native 런타임 오류 0건.
- Observe: 앱을 백그라운드로 보내자 Samsung Freecess가 2분 뒤 앱을 동결(`Initial -> Freezeable -> Frozen, Reason: uidIdle`)하고 AlarmManager가 예약 알람 3건을 `Deferring alarm ... due to freecess`로 보류해, 짐 챙기기 1분 전·종료·출발 알림이 예정 시각(17:31:56·17:32:56)에 오지 않고 앱을 다시 연 17:36:10에 한꺼번에 전달됐다. 전면 상태에서도 `setInexact` 예약이라 1분 전 알림이 44초 늦게 온 사례가 있었다(17:24:19 예약 → 17:25:03 전달). APK에 `SCHEDULE_EXACT_ALARM`/`USE_EXACT_ALARM`이 없어 expo-notifications가 비정확 알람을 쓰는 것이 원인이며, 백그라운드 단계 종료 알람의 신뢰성은 별도 결정(정확 알람 권한 또는 배터리 최적화 예외 안내)이 필요하다. → 같은 날 다음 항목에서 정확 알람 권한·배터리 최적화 예외 카드로 해결했다. 알림 본문 탭으로 완료되는 경로와 지난 알림 탭 무시 경로는 실기기에서 사용자가 앱 안에서 알람을 먼저 꺼 이번에는 확인하지 못했다.
- Evidence: `src/app/progress.tsx`, `src/lib/local-notifications.ts`, `src/lib/notification-service.ts`, `src/lib/confirmed-plan-notification-service.ts`, `src/lib/progress-session.ts`, `src/lib/voice-schedule-assistant.ts`, `src/app/voice-schedule.tsx`, `src/state/schedule-context.tsx`, `assets/sounds/step-alarm.wav`, `src/lib/__tests__/local-notifications.test.ts`, `src/lib/__tests__/progress-session.test.ts`, `src/lib/__tests__/voice-schedule-assistant.test.ts`, `tmp/step-alarm-auto-open.png`, `tmp/step-alarm-overdue.png`, `tmp/step-alarm-next-step.png`, `tmp/voice-transport-choice.png`, `tmp/voice-transport-followup.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-21 정확한 시각 알람·배터리 최적화 예외·종료 알람 우선 (완료)

- Accept: 앱을 닫아 두고 기기가 앱을 절전 동결해도 준비 단계 종료 알람이 예정 시각에 알람 소리·진동과 함께 도착한다. 사용자는 무엇이 알람을 늦출 수 있는지 텍스트로 보고, 설명을 읽은 뒤 자기 손으로 시스템 설정을 연다. 설정을 하지 않아도 앱을 열어 두면 기존 안내는 그대로다.
- Implement: 로컬 Expo 모듈 `modules/on-time-alarm-access`(Android)를 추가해 `canScheduleExactAlarms`·`isIgnoringBatteryOptimizations`와 `ACTION_REQUEST_SCHEDULE_EXACT_ALARM`·`ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`(실패 시 목록 화면) 열기를 제공하고, `app.json`에 `SCHEDULE_EXACT_ALARM`·`REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`를 선언해 expo-notifications가 `setExactAndAllowWhileIdle`을 쓰게 했다. `src/lib/alarm-reliability.ts`가 두 상태를 읽어 텍스트 라벨·요약·진행 화면 힌트를 만들고, 권한 화면의 `준비 알람 정확도` 카드(설명 → 상태 배지 → 설정 열기 버튼, 복귀 시 재확인), 설정의 `알림 및 권한 > 준비 알람 정확도` 행, 진행 화면 알림 상태 아래의 `알람 설정 확인` 힌트로 노출한다. 실기기에서 단계 종료 알람(알람 채널)과 다음 단계 시작·출발 안내(일반 채널)가 같은 밀리초에 게시되어 먼저 온 일반 알림이 소리를 가져가고 알람은 소리·진동 없이 끝나는 것을 발견해, `FOLLOW_UP_CUE_DELAY_MS = 4_000`으로 후속 안내를 종료 알람 4초 뒤로 미뤘다.
- Verify: `npm run verify` TypeScript·ESLint·Jest 47개 스위트·486/486 통과(라이브러리 6건, 후속 안내 지연 1건 추가). 업로드 키 서명 arm64 release APK 772 tasks 빌드를 Samsung Android 12 `SM-N971N`에 두 차례 덮어 설치했다. 1차(SHA-256 `c8ebc1c3e6b10446761b3208ba2f8e89742c80771026022708b0ed2212a00d91`): 권한 카드가 `정확한 시각 알람 허용됨`·`배터리 최적화 예외 적용됨`을, 설정 행이 `단계 종료 알람이 제시간에 울려요`를 표시했고, 예약 알람이 `dumpsys alarm`에서 모두 `window=0 exactAllowReason=permission`으로 바뀌었다. 앱을 백그라운드로 보내 Freecess가 19:27:42에 동결한 뒤에도 19:29:33.170·19:30:33.170·19:32:33.170·19:36:33.170·19:37:33.170에 알람이 예정 시각 ±0.2초로 전달됐고 `Deferring` 로그는 없었다. 이때 NMS `mSoundNotificationKey`가 매번 같은 순간의 일반 채널 알림(`… 시작할 시간이에요`·`이제 출발할 시간이에요`)을 가리키고 `mVibrateNotificationKey`는 준비 시작에 머물러 종료 알람이 소리·진동 없이 게시되는 문제를 확인했다. 2차(SHA-256 `3ac59e35e6751acb`로 시작): 동결(20:01:03) 상태에서 20:02:54.823에 `샤워 예정 시간이 끝났어요`가 도착해 SystemUI RingtonePlayer가 `USAGE_ALARM` 소리를 20:02:55.2~58.6에 재생하고 `VibratorManagerService`가 20:02:55.9에 진동했으며, `옷 입기 시작할 시간이에요`는 4초 뒤 20:02:58.8에 따로 게시됐다. 치명적 런타임 오류 0건.
- Observe: 이 기기는 이미 배터리 최적화 예외 목록에 있었는데도 Freecess가 2분 뒤 앱을 동결했으므로, Samsung에서 실제로 알람을 살린 것은 정확 알람 권한이다. 배터리 예외 카드는 비정확 알람만 가능한 경우의 보조 수단으로 남긴다. `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`의 Play 정책 근거는 `docs/PLAY_DECLARATIONS.md`에 적었다.
- Evidence: `modules/on-time-alarm-access/`, `src/lib/alarm-reliability.ts`, `src/lib/__tests__/alarm-reliability.test.ts`, `src/lib/local-notifications.ts`, `src/lib/__tests__/local-notifications.test.ts`, `src/app/permissions.tsx`, `src/app/settings.tsx`, `src/app/progress.tsx`, `app.json`, `package.json`, `docs/PLAY_DECLARATIONS.md`, `docs/HARNESS.md`, `tmp/alarm-reliability-card.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-23 약속 시간 휠·달력 날짜·요일 반복 (완료)

- Accept: 수작업 등록(`/create`) 1단계에서 약속 시간은 갤럭시 알람처럼 `오전/오후`·`시`·`분` 세 개의 드럼을 위아래로 밀어 고르고, 손을 떼면 값이 가운데 띠에 정확히 맞춰지며 바로 아래 문장(`오후 1:35 · 위아래로 밀어서 고르세요`)으로 다시 읽힌다. 날짜는 `오늘 · 8월 23일 (일)` 행을 눌러 펼쳐지는 월 달력(이전·다음 달, `오늘`, 지난 날짜 비활성)에서 고른다. 요일 칩(일~토)과 `매일`·`평일`·`주말` 프리셋으로 매주 반복을 설정할 수 있고, 요일을 켜면 날짜가 그 요일의 첫 날로 이동하며 달력에서 반복 요일이 아닌 날을 고르면 한 번만 하는 약속이 된다. 반복 약속은 홈·일정 카드에 `매주 화 반복`으로 표시되고, 그 회차가 끝나거나 지나가면 같은 시간의 다음 회차가 알람 예약과 함께 자동으로 등록된다(다음 회차를 삭제하면 반복이 멈춘다). 지난 시각으로는 다음 단계로 갈 수 없다. 각 드럼은 보조 기술에 하나의 조절 컨트롤(증가/감소)로 읽히고, 값은 탭으로도 고를 수 있다. 색만으로 상태를 전달하지 않는다(띠·굵기·요약 문장·취소선).
- Implement: 순수 로직을 `src/lib/appointment-date.ts`(시계 ↔ 드럼 변환, `YYYY-MM-DD (요일)` 저장 형식과 `오늘 · 8월 23일 (일)` 표시, 모든 기존 날짜 텍스트를 읽는 `resolveAppointmentDate`, 6주 달력 격자)와 `src/lib/appointment-recurrence.ts`(요일 목록 정규화·토글·프리셋·`매주 월·수·금` 요약, 음성 문장 `매주 화, 목`/`평일마다` 파싱, 오늘 시각이 지났으면 다음 주로 넘기는 `nextRepeatDate`)에 두고, `confirmed-plans.ts`의 `resolveScheduleDateTime`이 같은 파서를 쓰게 했다. 초안에 `repeatWeekdays`(0=일…6=토)를 추가하고 `recurrence` 문장을 함께 갱신해 음성 흐름과 호환을 유지했다. 확정 계획에는 `seriesId`를 두고, `spawnNextRecurringPlans`가 완료·미완료로 닫힌 반복 회차마다 다음 회차를 정확히 한 번 만든다(뒤 회차가 이미 있으면 만들지 않고, 앱을 오래 안 열었으면 지난 주를 재생하지 않고 지금 이후 첫 요일로 잡는다). `schedule-context`의 settle 흐름이 이를 호출해 새 회차의 준비 시작 알람을 예약한 뒤 저장한다. UI는 `src/components/wheel-picker.tsx`(`WheelColumn`·`TimeWheelPicker`: ScrollView snap + 정지 후 140ms 정착 커밋, 드래그 중 커밋 금지, 마운트 시 위치 지정, `adjustable` 역할·`aria-valuetext`·increment/decrement 액션, 탭 선택)와 `src/components/appointment-date-picker.tsx`(날짜 행·접이식 달력·요일 칩·프리셋·요약 문장)로 만들었고, `create.tsx`의 날짜·시간 텍스트 입력을 이들로 바꾸고 지난 시각 검증을 추가했다. 반복 약속일 때 시간을 바꾸면 날짜도 첫 반복 요일로 따라간다. 음성 확인 화면의 날짜 행은 저장 형식을 `8월 27일 (목)`으로 읽어 보여준다. 홈·일정 카드에 반복 줄을 추가했다.
- Verify: `npm run verify` TypeScript·ESLint(오류 0)·Jest 50개 스위트·520/520 통과(appointment-date 10건, appointment-recurrence 8건, confirmed-plans 반복 5건, schedule-draft 2건, 컴포넌트 `appointment-pickers` 9건 추가). Playwright 시각 회귀 360×800·390×844·430×932 3개 뷰포트에서 `create-step-1-default`·`create-destination` 기준선을 갱신하고 `create-calendar-open`·`create-repeat-friday`를 추가했으며, 휠을 5칸 밀어 `오후 1:40`, 달력에서 7월 30일 선택, `금` 칩으로 7월 31일 (금)·`매주 금`·초안 저장(`2026-07-31 (금)`, `13:40`, `[5]`)까지 브라우저에서 확인했다(가로 넘침 0). 업로드 키 서명 arm64 release APK(772 tasks, `BUILD SUCCESSFUL`, SHA-256 `faddec93d0009ce6c96e2a2c6ff92b3d48c624b503846e93fd1af579843c6e3e`)를 Samsung Android 12 `SM-N971N`에 덮어 설치하고 `ontime://create?new=1`로 열어 01:17~01:20에 확인했다: 드럼이 `오전 1 : 50`으로 띠에 맞춰 표시되고 분 드럼을 위로 밀자 `오전 1:53`으로 정착했으며, 날짜 행을 누르자 `2026년 8월` 달력이 펼쳐지고 지난 날짜는 취소선으로 비활성, `26`을 누르자 달력이 닫히며 `8월 26일 (수)`로, `화` 칩을 켜자 `9월 1일 (화)`·`매주 화`로, `평일` 프리셋은 `평일마다`로 바뀌었다(`maestro hierarchy`의 accessibilityText로 확인). 실기기 확인 뒤 음성 화면 날짜 표시 보강을 포함해 다시 빌드한 최종 APK(772 tasks, `BUILD SUCCESSFUL`, SHA-256 `428518c6ba353fe54c0567e3b745cb4da77e9c16f4d11ceb53fa37b394731949`)도 같은 기기에 덮어 설치해 등록 화면이 `오전 1:55` 드럼과 `오늘 · 8월 23일 (일)` 행으로 열리는 것을 확인했다(01:25).
- Observe: Metro 개발 서버가 파일 변경을 다시 번들하지 않아 웹 검증 초반에 옛 번들을 보고 있었다(`--clear`로 재시작 후 해결). `page.clock.install`이 걸린 Playwright에서는 마운트 직후 드럼 위치 지정이 꼭 필요하다(React Native Web ScrollView는 `contentOffset`을 무시), 초안 복원처럼 바깥에서 값이 바뀔 때는 부드러운 스크롤 대신 즉시 이동해야 스크린샷이 결정적이다(390×844 `create-destination`이 분 드럼 애니메이션 도중 찍혀 한 번 어긋났다). RNTL 14는 `render`·`fireEvent`가 비동기라 컴포넌트 테스트는 `await`로 쓴다. 아이콘 패키지(`lucide-react-native`)는 ESM이라 컴포넌트 테스트에서 `AppIcon`을 목으로 대체한다.
- Evidence: `src/lib/appointment-date.ts`, `src/lib/appointment-recurrence.ts`, `src/lib/__tests__/appointment-date.test.ts`, `src/lib/__tests__/appointment-recurrence.test.ts`, `src/lib/confirmed-plans.ts`, `src/lib/__tests__/confirmed-plans.test.ts`, `src/lib/schedule-draft.ts`, `src/lib/__tests__/schedule-draft.test.ts`, `src/components/wheel-picker.tsx`, `src/components/appointment-date-picker.tsx`, `src/components/__tests__/appointment-pickers.test.tsx`, `src/app/create.tsx`, `src/app/index.tsx`, `src/app/schedules.tsx`, `src/app/voice-schedule.tsx`, `src/state/schedule-context.tsx`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/create-*.png`, `docs/HARNESS.md`(42번), `tmp/picker-device-1.png`~`tmp/picker-device-8.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-23 홈 인사말 제거·하루 한 번 준비 행동·내비게이션 중앙 음성 버튼 (완료)

- Accept: 홈 헤더에 `좋은 새벽이에요, WOOCHUL님` 같은 인사말이 없고 날짜·약속 수가 제목으로 선다. 같은 날 이미 확정된 약속보다 늦은 약속을 만들면 샤워·스킨케어·화장·면도·헤어 정돈처럼 하루 한 번이면 되는 준비 행동은 그날 첫 일정에만 남고, 두 번째 약속의 준비 행동 단계에는 `오늘 두 번째 약속이에요` 카드와 어느 일정에서 마치는지 한 문장, 항목별 `다시 넣기` 칩이 보인다(최소 한 항목은 남긴다). 음성으로 바로 확정한 약속에도 같은 규칙이 적용된다. 음성 일정 버튼은 하단 내비게이션 가운데에 바의 윗변을 중심으로 반쯤 걸친 채 고정되어 모든 탭에서 같은 자리에 있고, 홈 오른쪽 아래의 떠 있던 버튼은 없앤다.
- Implement: `src/lib/home-greeting.ts`와 테스트를 지우고 `index.tsx` 헤더를 날짜 줄(`8월 23일 일요일 · 오늘·내일 약속 1개`, 17px 굵은 제목)만 남겼다. `src/lib/daily-routines.ts`에 `ONCE_PER_DAY_ROUTINE_IDS`, 같은 날 더 이른 확정 약속을 찾는 `findEarlierSameDayPlan`(수정 중인 약속 제외), 자리 정보를 남기며 빼는 `dropOncePerDayRoutines`, `applyOncePerDayRule`, `restoreOncePerDayRoutine`, 문장 `describeOncePerDaySkip`(`withObjectParticle` 사용)을 두었다. `create.tsx`는 이동 정보 → 준비 행동으로 넘어갈 때 규칙을 한 번 적용해 초안 루틴을 줄이고 카드·칩을 보여 주며, `schedule-context`의 `confirmScheduleDirectly`(음성 직접 확정)도 같은 규칙을 거친다. `bottom-nav.tsx`는 탭 2개·가운데 슬롯·탭 2개로 바꾸고 `VoicePulseButton`(60px)을 바 윗변 중심에 절대 배치했다(펄스 링 박스 `VOICE_PULSE_RING`을 고려해 정확히 절반이 올라오도록, 배경색 우물로 콘텐츠와 분리). `bottom-navigation-layout.ts`의 홈 전용 플로팅 상수는 `BOTTOM_NAV_ACTION_SIZE`·`BOTTOM_NAV_ACTION_OVERLAP`·`getBottomNavigationActionReach`로 바꿨다.
- Verify: `npm run verify` TypeScript·ESLint(오류 0)·Jest 50개 스위트·516/516 통과(daily-routines 6건 추가, 레이아웃 3건 갱신, 인사말 테스트 삭제). Playwright 시각 회귀 3개 뷰포트 234/234 통과 — `home`·`dark-home`·`home-on-time-streak`·`alerts`·`calendar-events`·`settings`·`dark-settings`·`dark-schedules`·`mvp-metrics` 기준선 갱신, `create-second-of-day` 추가, 음성 버튼이 가로 중앙이고 중심이 바 윗변과 3px 이내인지 재는 테스트와 같은 날 두 번째 약속(14:00 확정 뒤 16:00 초안)에서 `샤워·화장`이 빠지고 `샤워 다시 넣기`로 돌아오는 테스트를 추가했다. 업로드 키 서명 arm64 release APK(772 tasks, `BUILD SUCCESSFUL`, SHA-256 `dabfd8c92c3bdf50024ef874c67bba39f408952493800f15c24c932830494af3`)를 Samsung Android 12 `SM-N971N`에 덮어 설치하고 02:10~02:20에 확인했다: 홈 제목이 `8월 23일 일요일 · 오늘·내일 약속 1개`로만 보이고 인사말이 없으며, 음성 버튼이 내비게이션 가운데 바 윗변에 반쯤 걸쳐 있고 누르면 음성 일정 화면이 열렸다. 12:57 약속이 확정된 상태에서 `ontime://create?new=1`로 새 약속을 열어 드럼을 `오후 2:45`로 바꾸고 준비 행동 단계로 가자 `오늘 두 번째 약속이에요`·`오늘 첫 일정(12:57 일요일 오전 약속)에서 샤워를 마치니 이번 계획에서는 뺐어요.`가 보이고 샤워가 빠졌으며 `샤워 다시 넣기`를 누르자 샤워가 돌아왔다(`maestro hierarchy` 확인).
- Observe: Playwright `-g`는 프로젝트·파일 경로를 포함한 전체 제목에 매칭하므로 `^home 화면`은 아무것도 고르지 않는다(`home 화면`으로 지정). 도킹 버튼은 펄스 링 때문에 버튼 박스가 버튼보다 18px 크므로 박스 높이의 절반만큼 올려야 정확히 반이 걸친다.
- Evidence: `src/lib/daily-routines.ts`, `src/lib/__tests__/daily-routines.test.ts`, `src/lib/bottom-navigation-layout.ts`, `src/lib/__tests__/bottom-navigation-layout.test.ts`, `src/components/bottom-nav.tsx`, `src/components/voice-pulse-button.tsx`, `src/app/index.tsx`, `src/app/create.tsx`, `src/state/schedule-context.tsx`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/`, `docs/HARNESS.md`(43·44번), `tmp/nav-device-1.png`, `tmp/nav-device-2.png`, `tmp/second-of-day-device-1.png`, `tmp/second-of-day-device-2.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-23 하루 한 번 준비 행동 규칙 강화 (완료)

- Accept: 샤워·화장(그리고 스킨케어·면도·헤어 정돈)은 하루에 한 번이다. 같은 날 확정된 다른 약속의 준비에 그 행동이 이미 있으면 — 새 약속이 더 늦든 더 이르든 — 새 약속에는 넣지 않고, 이 규칙은 준비 행동 단계뿐 아니라 계획을 만드는 순간(`AI 계획 만들기`, 음성 직접 확정)에도 적용되어 어떤 경로로도 두 번째 계획에 샤워가 들어가지 않는다. 사람이 `다시 넣기`로 일부러 되돌린 행동만 남는다.
- Implement: `daily-routines.ts`를 "더 이른 약속이 있으면"에서 "같은 날 다른 약속이 그 행동을 이미 가지고 있으면"으로 바꿨다(`findSameDayPlans`, `coveredOncePerDayRoutineIds`, 행동별로 판단하는 `dropOncePerDayRoutines`). `RoutineDraft.keepOnSameDay`를 추가해 `restoreOncePerDayRoutine`이 되돌린 행동에 표시하고 규칙이 건너뛰게 했으며, `schedule-context`의 `finalizeSchedule`(계획 생성)에서도 규칙을 적용한다. 안내 문장은 `오늘 12:57 볼링 준비에 샤워·화장이 이미 있어 이번 계획에서는 뺐어요.`로 바꿨다. 사용자가 본 "모든 일정에 샤워가 들어감"은 규칙이 준비 행동 단계 진입에만 걸려 있었던 점과, 앞선 기기 확인에서 `샤워 다시 넣기`를 누른 초안이 남아 있던 점이 겹친 결과다.
- Verify: `npm run verify` Jest 50개 스위트·519/519 통과(daily-routines 9건). Playwright `create-second-of-day` 3개 뷰포트 갱신·통과. arm64 release APK(`BUILD SUCCESSFUL`, SHA-256 `f687f0966fff113888c0533e1b58e4e10783413123ecad76fef5d0c0810a2ed9`)를 `SM-N971N`에 덮어 설치하고 `ontime://create?new=1`로 새 초안을 열어(12:57 약속이 확정된 상태) `오후 3:10`으로 바꾼 뒤 준비 행동 단계에서 `오늘 두 번째 약속이에요`·`오늘 12:57 일요일 오전 약속 준비에 샤워가 이미 있어 이번 계획에서는 뺐어요.`와 샤워 제외를 확인하고, `AI 계획 만들기`로 만든 계획에도 옷 입기·짐 챙기기만 있고 샤워가 없음을 확인했다(03:1x).
- Evidence: `src/lib/daily-routines.ts`, `src/lib/__tests__/daily-routines.test.ts`, `src/lib/schedule-draft.ts`, `src/state/schedule-context.tsx`, `e2e/visual/app.visual.spec.mjs`, `docs/HARNESS.md`(44번), `tmp/second-of-day-device-3.png`, `tmp/second-of-day-device-4.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-24 홈 개편·날씨 이동·일정 달력·이동수단 통합·5분 전 알림 (완료)

- Accept: (1) 홈 상단의 날짜·약속 수 헤더 줄이 없고, 약속 상자는 제목(+연필 아이콘), `12:55 (6분 지남) / 14:00` 형태의 준비 시작/약속 시간 한 줄(약속 시간 폰트 크기 유지), 시간 옆의 원형 `시작` 버튼만 담는다. 연필은 준비 계획 상세를 열고, 상자와 `시작`은 실시간 준비 화면을 연다. `오늘의 준비 계획`은 준비 계획 상세와 같은 전체 타임라인(이동수단 단계 포함)을 보여준다. (2) 현재 위치 날씨는 홈에서 빠지고 알림 화면 맨 위로 이동한다(권한·오류 복구 행동과 상세 이동 포함). 확정 약속이 없는 홈은 `말로 새 일정 만들기` 버튼 없이 `일정을 등록하면 준비 시작 시각에 자동으로 실행돼요.` 한 줄의 안내 상자 하나이며, 상자를 누르면 일정 생성 화면이 열린다(준비 시각 자동 실행은 기존 `startProgress('auto')`가 이미 수행). (3) 일정 화면 `내 일정` 탭은 상단은 그대로 두고 폰 기본 달력식 월 달력(일정 있는 날 점, 오늘 링, 날짜 선택 시 그 날의 일정 카드)을 보여주며, 텍스트 줄 오른쪽 끝의 `+`가 직접 입력 등록을 열고 탭의 음성 버튼은 없앤다. (4) 이동수단은 `AI 추천·대중교통·승용차(택시)·도보`로 통합되고(설정은 3택), 저장된 버스·지하철·택시·자가용은 각각 대중교통·승용차(택시)로 읽히며, 경로 조회는 기존 세부 수단으로 물어 통합 수단 안에서 빠른 쪽을 쓴다. 출발 단계는 `대중교통으로 출발`/`승용차로 출발`이다. 남성 프로필의 기본 준비 항목은 화장 없이 면도다(회귀 테스트로 고정). (5) 준비 시작 5분 전에 `약속 준비 시작 5분 전이에요` 알림(소리)과, 앱이 열려 있으면 같은 문장의 음성 안내가 나온다. 약속 삭제 시 5분 전 알림도 취소된다.
- Implement: `index.tsx` 헤더 줄·날씨·`지금 시작` 버튼 제거, 새 hero(제목+연필 `edit` 아이콘, 시간 한 줄 `heroTimes`, 원형 `시작`), 타임라인 slice 제거·`transport` 전달, 빈 상자 Pressable화. `weather-card.tsx`로 날씨 상태·로딩·UI를 옮겨 `alerts.tsx` 상단에 배치, `home-greeting`은 이미 삭제된 상태에서 `home-logo-button.tsx`·`shouldAnimateHomeLogo`도 제거. `plan-calendar.tsx`(월 격자·점·오늘 링)를 만들어 `schedules.tsx` `내 일정` 탭에 붙이고 선택일 필터(`plansForLocalDate`)와 `+`(IconButton primary) 추가, 탭의 음성 버튼 삭제. `TransportMode`를 4종으로 바꾸고 `parseTransportModeText`/`resolveTransportMode`가 옛 값·말한 단어를 접으며, 초안·설정 로더가 저장된 옛 값을 이관한다. `planning.ts` 속도표·기본치(대중교통 26분, 승용차 19분)와 출발 단계 문구, `travel-estimate.ts`는 통합 수단을 세부 수단(버스·지하철/자가용·택시) 답 중 빠른 것으로 해석(서버·모빌리티 API는 무변경). 음성 비서의 선택지·검증·즉답 파싱도 통합 수단으로 바꿨다. `confirmed-plan-notification-service`가 시작 알람과 함께 5분 전 알림(`describePrepStartReminder`, 알람 채널, `reminderNotificationIdentifier` 저장·취소)을 예약하고, `schedule-context`에 전면 상태에서 한 번만 말하는 음성 예고 효과를 추가했다(`canUseAppTts`·알림 설정 존중).
- Verify: `npm run verify` TypeScript·ESLint(오류 0)·Jest 50개 스위트·520/520 통과(레거시 이동수단 이관, 남성 기본 항목, 통합 수단 계획 수치 재계산 등 갱신). Playwright 시각 회귀 3개 뷰포트 234/234 통과 — 홈 약속 상자(가운데 정렬 원형 시작·연필→상세·상자→진행 화면), 빈 홈 상자, 일정 달력, 이동수단 3택(설정·음성), 알림 날씨 이동 테스트를 갱신·추가하고 home/dark-home/streak/alerts/dark-schedules/voice-schedule-transport/personalized-plan 기준선을 갱신했다. 업로드 키 서명 arm64 release APK(`BUILD SUCCESSFUL`, SHA-256 `2463673f27afa425fc68ca5122f77dc9df687ed3496318a78672fee66e8e308c`)를 `SM-N971N`에 설치해 15:19~15:5x에 확인했다: 홈 상자 `17:19 (1일 뒤 시작) / 17:55`+원형 `시작`+연필(→확정된 준비 계획), 헤더 줄 없음, 알림 맨 위 `해운대구 · 맑음 32°` 날씨 카드, 일정 탭의 `2026년 8월` 달력과 `+`(→등록 화면), 이동수단 그리드가 정확히 `AI 추천·대중교통·승용차(택시)·도보`, 17:00 약속 확정 시 타임라인 `대중교통으로 출발 26분`과 `dumpsys alarm`의 정확 알람 2건(15:57 리마인더·16:02 시작, 모두 `window=0 exactAllowReason=permission`), 같은 날 세 번째 초안에서 `오늘 두 번째 약속이에요` 카드 재확인. 15:57:00 예약분은 15:57에 실제로 도착해 `android.title=약속 준비 시작 5분 전이에요`와 `mSoundNotificationKey=0|com.timeagent.app|…`(소리 재생)가 `dumpsys notification --noredact`에 남았다.
- Observe: 준비 시각 자동 실행(요구 6)은 기존 `startProgress('auto')` 경로가 이미 구현하고 있어 코드 변경 없이 확인만 했다. `personalized-plan` 시각 테스트는 같은 날 확정 약속 픽스처와 겹치면 하루 한 번 규칙이 샤워를 빼 개인화 배너가 사라지므로 확정 목록을 비우고 돌린다.
- Evidence: `src/components/weather-card.tsx`, `src/components/plan-calendar.tsx`, `src/components/bottom-nav.tsx`, `src/components/voice-pulse-button.tsx`, `src/components/app-icon.tsx`, `src/app/index.tsx`, `src/app/alerts.tsx`, `src/app/schedules.tsx`, `src/app/create.tsx`, `src/app/settings.tsx`, `src/app/voice-schedule.tsx`, `src/lib/schedule-draft.ts`, `src/lib/planning.ts`, `src/lib/travel-estimate.ts`, `src/lib/app-settings.ts`, `src/lib/voice-schedule-assistant.ts`, `src/lib/confirmed-plans.ts`, `src/lib/confirmed-plan-notification-service.ts`, `src/lib/home-attention.ts`, `src/state/schedule-context.tsx`, 각 `__tests__`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/`, `docs/HARNESS.md`(43 갱신·45~49), `tmp/final-*.png`, `tmp/reminder-fired.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-24 홈 타임라인 구역 삭제·등록 약속 제목의 + 버튼 (완료)

- Accept: 홈에 `오늘의 준비 계획` 타임라인 구역이 없다(전체 타임라인은 준비 계획 상세에서 본다). `오늘·내일 등록 약속 N개` 제목 오른쪽의 `전체 보기` 링크를 없애고, 그 자리에 일정 화면과 같은 원형 `+`(새 일정 직접 등록 → 등록 화면) 버튼을 둔다.
- Implement: `index.tsx`에서 타임라인 구역과 `todayPlans`/`nextTodayPlan` 계산, `전체 보기` 링크를 제거하고 SectionTitle의 action에 `IconButton(plus, primary)`를 넣었다. 미사용이 된 `Timeline`·`Button`·`useAppType` 정리.
- Verify: `npm run verify` 50개 스위트·520/520 통과. 시각 회귀에서 home·dark-home·home-on-time-streak 기준선을 갱신하고 홈 관련 테스트 7건(약속 상자·빈 홈·캘린더 비표시 등) 통과, 캘린더 비표시 테스트에 `오늘의 준비 계획` 부재 단언을 추가했다. 시각 회귀 전체 3개 뷰포트 234/234 통과. arm64 release APK(SHA-256 `0e9e72aad717880f67166c5dc941cc4be165472309bdab205fadc264b6b56cdf`)를 SM-N971N에 설치해 홈에 타임라인 구역과 전체 보기가 없고 제목 오른쪽 + 버튼이 등록 화면을 여는 것을 확인했다.
- Evidence: `src/app/index.tsx`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/home*.png`, `docs/HARNESS.md`(45 갱신).

## 2026-08-24 시계 앱식 홈: 무박스 알람 안내·큰 시간·계획별 알람 스위치 (완료)

- Accept: 홈 상단은 상자 없이 갤럭시 시계처럼 `31분 후에 준비 알람이 울려요`와 `8월 24일 (일) 오후 4:02 · 약속 이름`을 보여주고, 원형 `시작` 버튼은 우측 최상단에 유지된다(문구·버튼 모두 실시간 준비 화면). 약속 목록의 약속 시간 폰트는 두 배(18→36px)이고, 각 항목 오른쪽의 알람 스위치로 그 약속의 준비 알람을 켜고 끌 수 있다 — 끄면 예약된 시작·5분 전 알림이 취소되고 자동 시작·음성 예고에서도 빠지며 `알람 꺼짐`으로 흐려지고, 켜면 재예약된다. 전부 꺼져 있으면 상단이 `켜진 준비 알람이 없어요`가 된다.
- Implement: `home-attention.ts`에 `describeNextAlarm`(남은 시간 단위 전환·`8월 24일 (일) 오후 4:02` 표기), `confirmed-plans.ts`에 `alarmEnabled`(기본 켬)·`isPlanAlarmEnabled`·알림 id를 함께 지우는 `setPlanAlarmEnabled`를 추가하고, 알림 서비스는 꺼진 계획을 예약하지 않는다. `schedule-context`에 `setPlanAlarmEnabled`(끄기: 취소→저장, 켜기: 재예약→저장)를 노출하고 자동 시작·음성 예고 효과가 꺼진 계획을 건너뛴다. `index.tsx`는 hero 카드를 무박스 안내+우측 상단 `시작`으로 바꾸고, 목록 카드를 Pressable+`Switch` 나란 구조로 재구성했다(시간 레일 122px·36px 숫자, 꺼짐 시 opacity 0.45).
- Verify: `npm run verify` 50개 스위트·524/524 통과(알람 스위치 2건, 알람 안내 2건 추가). 시각 회귀: 홈 상단 무박스·시작 버튼 우측 상단 위치·스위치 토글 저장/복원 테스트 추가, home·dark-home·streak 기준선 갱신, 전체 3개 뷰포트 스위트 통과(수치는 실행 로그 참조). 실기기: 사용자 승인 후 새 폰 SM-S931N(Android)의 Play 서명 1.0.9를 제거하고 arm64 release APK(SHA-256 `44e36c9cf0f7349978d18cbf6c55f10bfbeeb1b221613767ffa057fc576b7902`)를 설치, Google 로그인 뒤 17:52~18:00에 확인했다: 실제 18:30 약속(실행 중)에서 홈 상단이 상자 없이 `준비를 진행하고 있어요`·`8월 24일 (월) 오후 5:53 · 월요일 오후 약속`과 우측 상단 원형 `시작`으로 표시되고, 목록의 18:30이 큰 숫자로 보였다(maestro hierarchy·스크린샷). 다음 날(8/25) SM-N971N에 같은 APK를 재설치해 스위치를 실검증했다: 홈 상단이 `5시간 40분 후에 준비 알람이 울려요`로 뜨고 매주 월·화 반복이 당일 17:55 회차를 자동 생성했으며, 스위치를 끄자 `알람 꺼짐`·`켜진 준비 알람이 없어요`와 함께 예약 알람 0건, 다시 켜자 16:57(5분 전)·17:02(시작) 두 건이 `window=0 exactAllowReason=permission`으로 재등록되고 상단 문구가 돌아왔다(`dumpsys alarm`, tmp/switch-off.png·tmp/switch-on.png).
- Evidence: `src/lib/home-attention.ts`, `src/lib/__tests__/home-attention.test.ts`, `src/lib/confirmed-plans.ts`, `src/lib/__tests__/confirmed-plans.test.ts`, `src/lib/confirmed-plan-notification-service.ts`, `src/state/schedule-context.tsx`, `src/app/index.tsx`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/home*.png`, `docs/HARNESS.md`(45 갱신), `tmp/alarm-home-*.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-25 랜딩 페이지 Play 링크·TimeAgent 리브랜딩 재배포 (완료)

- Accept: `timeflow.html` 랜딩의 CTA(무료로 시작하기) 링크 3곳이 닫힌 테스트 옵트인 URL 대신 Play 스토어 상세(`https://play.google.com/store/apps/details?id=com.timeagent.app`)로 가고, 페이지의 `TimeFlow` 표기 3곳(후기·로고·저작권)이 모두 `TimeAgent`가 된다. 안정 도메인에서 반영을 확인한다.
- Implement: `sites/timeflow-landing/public/timeflow.html`에서 두 치환을 수행했다. 배포는 네 차례 막혔는데, 처음 두 번은 대용량 파일 업로드가 멈춰 `--archive=tgz`로 전환했고, 이후 모든 배포가 `BLOCKED`로 남는 원인을 REST API(`readyState=BLOCKED`, `seatBlock: TEAM_ACCESS_REQUIRED`)와 대시보드에서 확인했다 — 커밋 이메일 `gnsdudwkd@naver.com`이 Vercel 계정과 연결되지 않아 Hobby 정책이 차단(프로젝트는 `timeagnet`으로 개명돼 있었음). git 메타데이터가 없는 사본(스크래치 디렉터리, `.git` 제거)에서 배포해 차단을 우회했고, 첫 사본은 `build/`(vite 플러그인) 누락으로 타입 검사가 실패해 보완 후 성공했다.
- Verify: 배포 `READY`·프로덕션 승격 확인(중복 promote 시 409 "already current"). `https://timeflow-landing-vert.vercel.app/timeflow.html`과 `https://timeagnet-mangotree-4133s-projects.vercel.app/timeflow.html` 모두 HTTP 200에서 `TimeFlow` 0회·`TimeAgent` 4회·`apps/testing` 0회·스토어 상세 링크 3회를 확인했다.
- Observe: 이후 이 저장소에서 직접 배포하려면 Vercel 계정에 커밋 이메일(`gnsdudwkd@naver.com`)을 추가·인증하거나(대시보드 `Fix Git Configuration`), 이번처럼 git 컨텍스트 없는 사본에서 배포한다. UNKNOWN으로 보이던 상태는 구 CLI(59.5.0)가 `BLOCKED`를 매핑하지 못한 표기였다.
- Evidence: `sites/timeflow-landing/public/timeflow.html`, Vercel 배포 `timeagnet-k0e4sa1w1…`, `docs/EXECUTION_PLAN.md`.


## 2026-08-25 홈 상단 삼각형 시작 버튼·두 줄 알람 문구·약속 개수 줄 (완료)

- Accept: 홈 상단은 좌측 끝의 모서리 라운드 삼각형(▶) 시작 버튼, 오른쪽에 큰 글씨 두 줄(`1시간 6분 후` / `준비 알람이 울립니다`, 진행 중이면 `준비를 진행하고 있어요`), 아래 작은 글씨 한 줄로 `오늘 화요일 약속 1개 · 내일 1개` 개수를 보여준다. `+` 버튼 왼쪽의 `오늘·내일 등록 약속 N개` 제목은 삭제하고 `+`만 남긴다. 약속 카드의 상태 표시(실행 중 등)는 좌측 시간 아래로 이동한다.
- Implement: `describeNextAlarm`이 남은 시간 구(`31분 후`)를 따로 돌려주도록 바꾸고, `index.tsx` 상단을 react-native-svg의 라운드 조인 Path 삼각형 버튼 + 두 줄 헤드라인 + `plansForLocalDate` 기반 개수 줄로 재구성했다(모든 알람이 꺼진 상태도 같은 레이아웃). 목록 카드의 StatusPill을 시간 레일 아래로 옮겼다.
- Verify: `npm run verify` 50개 스위트·524/524 통과. 시각 회귀: 상단 좌측 삼각형·두 줄 문구·개수 줄 테스트로 재작성해 통과, ready 마커를 개수 줄로 교체, home·dark-home·streak 기준선 갱신, 전체 스위트 통과(수치는 로그). 실기기: arm64 release APK(SHA-256 `a2cc77dce30bbdf7974c0e8dbfc462029858baf4545ef0b4962e890d1dc03b09`)를 SM-N971N에 설치해 좌측 삼각형 시작 버튼·두 줄 문구·개수 줄을 확인했다(tmp/tri-home.png).
- Evidence: `src/lib/home-attention.ts`, `src/lib/__tests__/home-attention.test.ts`, `src/app/index.tsx`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/home*.png`, `docs/HARNESS.md`(45 갱신), `tmp/tri-home.png`, `android/app/build/outputs/apk/release/app-release.apk`.


## 2026-08-25 홈 상단·목록·진행 화면 마무리 다듬기 (완료)

- Accept: 홈의 삼각형 시작 버튼 아래 `시작` 라벨. 큰 글씨 줄바꿈은 `41분 후 준비` / `알람이 울립니다`, 진행 중에는 `약속 제목` / `진행하고 있어요`. 목록 카드의 상태 표시는 좌측 시간 아래 `오늘` 라벨 옆. 내비게이션 마이크 버튼 아래 `약속` 라벨. 진행(실시간 준비) 화면의 제목은 `실시간 준비` 대신 약속 제목.
- Implement: `index.tsx`(라벨·줄바꿈·헤드라인·상태 위치), `bottom-nav.tsx`(`약속` 라벨), `progress.tsx`(Header title=schedule.title, 로딩 상태만 기존 문구 유지). maestro 04 흐름의 `실시간 준비` 단언은 `단계 완료`로 교체.
- Verify: `npm run verify` 통과(수치는 로그), 시각 회귀 전체 재실행으로 기준선 갱신(홈·내비 라벨이 걸린 화면들·진행 화면). 실기기: arm64 release APK(SHA-256 `66dd42a22c9071e06aa83bf40b46c3f131db6a9c7a27f59abf88129f881e0510`)를 설치해 확인했다(tmp/final-tweaks-home.png).
- Evidence: `src/app/index.tsx`, `src/components/bottom-nav.tsx`, `src/app/progress.tsx`, `e2e/maestro/04_personalization_learning.yaml`, `e2e/visual/__screenshots__/*`, `docs/HARNESS.md`(43·45 갱신), `tmp/final-tweaks-home.png`, `android/app/build/outputs/apk/release/app-release.apk`.

## 2026-08-25 터치 시 사각형 음영 잔상 수정 (완료)
...
## 2026-08-26 대중교통 공급자 분리·시간표 기준 경로·실시간 도착정보 (완료)

- Accept: NAVER Maps는 지도·마커·경로선, TMAP은 장소·도보·자동차·택시·대중교통 경로와 미래 시각 운행시간표, TAGO는 선택 경로 첫 탑승 정류장의 버스 실시간 도착, NAVER 지도·카카오맵 연결은 상세 경로 확인과 공급자 장애 시 복구 행동을 담당한다. 계획 화면 `이동 시간 근거` 카드에 소요시간·요금·환승·도보와 `TMAP 시간표 기준/실시간 교통 기준/도보 경로/거리 기반 예상` 근거·계산 시각·출발 기준 시각이 텍스트로 보인다. 출발 30분 전부터 첫 탑승편을 20초 간격으로 확인하고 `실시간 · HH:MM 확인`/`최근 저장값 · HH:MM 확인`/`시간표 기준 · HH:MM 확인`과 이유 문장을 구분한다. 실시간이 계획 도착을 5분 넘게 늦추면 전후 시각과 `플랜 B에서 대안 보기`/`이대로 유지`를 제시하고 적용 전에는 계획·알림을 바꾸지 않는다.
- Select: 카카오맵 대중교통 API·ODsay는 도입하지 않고(PRD 결정 유지) 기존 Supabase `mobility` Edge Function에 endpoint를 추가했다. 지하철 실시간은 지역 adapter가 없어 일괄 `시간표 기준`으로 두고 외부 지도 확인을 제공한다. 실시간 변경안은 출발을 앞당기지 않고(같은 버스를 더 일찍 기다릴 뿐이므로) 늦어지는 도착만 투영해 사용자에게 플랜 B/유지를 맡긴다. 시간표에 포함된 평균 대기시간을 분리할 수 없어 tolerance 5분으로 일반 대기를 지연으로 오판하지 않게 했다.
- Implement: (서버) `supabase/functions/mobility/transit-contract.ts`(TMAP 대중교통 정규화: 구간·정류장 ID·도보·환승·요금·첫 탑승·KST `searchDttm`·5분 슬롯 캐시 키, 혼합 경로는 첫 탑승 모드로 경쟁·다른 모드로 위장 금지), `arrivals-contract.ts`(TAGO 정류장 좌표·이름 매칭 300 m/60 m, 노선 필터, 응답 코드 `22` 쿼터 분류), `short-cache.ts`(60초 경로·20초 도착 캐시, 실패 미저장, 동시 요청 공유), `provider-metrics.ts`(공급자·작업별 성공률·지연·호출량, 원문 위치·검색어 없음). `index.ts`에 `POST /v1/routes/estimates`(`departureAt` 수용, 과거 시각은 현재로), `POST /v1/routes/transit`(상세·경로선), `POST /v1/arrivals`(TAGO, 지하철·미설정·정류장 없음·노선 없음은 `unsupported`, timeout·429·5xx는 `unavailable`), `/health` 지표 노출. 상류 429/5xx는 예외로 올려 캐시에 남지 않는다. (클라이언트) `travel-estimate.ts` 계약 확장(`basis`·`walkMinutes`·`departureAt`·`firstBoarding`, `describeTravelEstimate`·`travelEstimateLabel`), `mobility-api.ts`(`departureAt` 전송, `getTransitRouteDetails`, `getTransitArrival`), `transit-arrival.ts`(30분 창·20초 재사용·마지막 유효값·자정 경계 `plannedDepartureAt`·변경안 계산), `transit-route.ts`(상세 경로→지도 경로선, 같은 모드만), `external-maps.ts`(nmap://·kakaomap:// 앱 링크와 웹 fallback), `planning.ts`의 `SchedulePlan.travelEstimate` 영속화(반복 다음 회차는 상속하지 않음), `schedule-context.tsx`(약속 일시 기반 `departureAt` 전달·시각 변경 시 재조회·같은 목적지 마지막 유효값 유지), `components/transit-evidence.tsx`(근거·첫 탑승·변경안·외부 지도 카드, 노선 변경 시 늦은 응답 폐기), `app/plan.tsx`(저장된 근거 우선, 대중교통 계획은 지도에 대중교통 경로선). 문서: `docs/INTEGRATIONS.md`·`docs/CLIENT_SECRET_SETUP.md`에 `TAGO_SERVICE_KEY`·새 endpoint, `docs/HARNESS.md` 수동 스모크 50.
- Verify: `npm run verify` 통과(56 suites / 574 tests, 린트 오류 0·기존 경고 8; 최종 실행 11:37). 새 테스트: `transit-contract`(정규화·혼합 경로·null 좌표·KST·캐시 키), `arrivals-contract`(TAGO 정류장·노선·거리 매칭), `short-cache`(TTL·동시 요청·실패 미저장·지표), `transit-arrival`(30분 창·20초 제한·실시간/최근 저장값/시간표 전환·변경안·자정), `transit-route`(모드 일치 선택·경로선 결합), `external-maps`, `mobility-api`(departureAt·firstBoarding·arrivals·상세 경로 파싱), `travel-estimate`(근거 라벨), `confirmed-plans`(반복 회차 근거 미상속). `deno check`(npx deno 2.9.5) `supabase/functions/mobility/index.ts` 통과. Codex(codex-cli 0.147.0) 교차 검증 2회: 1차 12건 결함 → 8건 해결·4건 부분 해결, 2차 신규 5건(자정 계산·반복 회차 근거 복제·360px 드럼 폭/접근성 라벨·혼합 경로 위장·다른 모드 지도) 모두 수정. 시각 회귀·실기기 관찰은 아래 Observe/Evidence.
- Deploy: 2026-08-26 12:31 `npx supabase functions deploy mobility --no-verify-jwt` → 프로젝트 `chpsoncuxjpgugowrydb` 배포 완료. `/health` → `{"status":"ok","realtimeArrivals":"not-configured"}`. `/v1/routes/estimates`(택시) 정상(`basis: "traffic"`, 요금 15,120원). 대중교통은 TMAP 직접 호출이 `429 QUOTA_EXCEEDED`(일일 한도)라 `/v1/routes/transit`이 `UPSTREAM_UNAVAILABLE`(503, retryable)로 정상 매핑됨 — 한도 초기화 뒤 재확인 필요. `TAGO_SERVICE_KEY`는 미등록(사용자 발급 필요).
- Observe: 실시간 도착은 TAGO 키(`TAGO_SERVICE_KEY`) 등록과 Edge Function 배포 뒤에 동작하며, 그 전에는 `/v1/arrivals`가 `not-configured`를 돌려 앱은 `시간표 기준`을 유지한다. 시간표의 평균 대기시간을 알 수 없어 변경안은 5분 이상 늦어질 때만 제시한다. 위치 권한 거부 시 이동 시간은 거리 기반 예상으로 남고 별도 안내는 계획 화면이 아닌 권한 화면이 담당한다(미해결 관찰).
- Evidence: `npm run visual:update` 237 passed(4.0m) — 갱신된 기준은 `plan`·`dark-plan`·`plan-map-open`·`create-step-3` 3개 폭 17개 파일뿐이며 360×800 `plan.png`에서 `이동 시간 근거` 카드가 잘림 없이 렌더됨(웹 fixture에는 경로 조회가 없어 빈 상태 문구만 보임 — 첫 탑승·실시간 상태의 시각 기준은 미확보). 실기기: arm64 릴리스 APK `BUILD SUCCESSFUL in 7m 20s`, 772 tasks, SHA-256 `191c2607f45ba37077be0d20e774932f4533187a1ba4ecf90afd5ee156392cf7`, 11:29 SM-N971N(무선 adb)에 `adb install -r` 성공. Edge Function은 미배포·TAGO 키 미등록 상태라 기기에서는 기존 서버 응답(근거·첫 탑승 없음)으로 카드 빈 상태와 외부 지도 버튼만 확인 가능하며, 실기기 관찰(12:13~12:16): 확정 계획 화면에 `이동 시간 근거` 카드·`전체 경로는 외부 지도에서 확인할 수 있어요`·`네이버 지도에서 보기`/`카카오맵에서 보기`가 표시됨(`tmp/tr-plan-1.png`). `네이버 지도에서 보기` 탭 → `com.nhn.android.nmap/com.naver.map.LaunchActivity`가 목적지 `롯데호텔 부산점` 대중교통 길찾기(38분)로 열림(`tmp/tr-naver.png`), `카카오맵에서 보기` 탭 → `net.daum.android.map/com.kakao.map.main.view.MainActivity` 열림(`tmp/tr-kakao.png`). 이 계획은 서버 배포 전 저장된 것이라 근거는 빈 상태 문구.

## 2026-08-26 준비 행동 시간을 좌우 드럼으로 조절 (완료)

- Accept: 새 일정 3단계의 각 준비 행동 시간은 `−`/`+` 버튼 대신 약속 시간 드럼과 같은 방식의 스와이프 드럼으로 고르되 방향은 좌우다. 손을 떼면 가운데 띠에 값이 맞춰지고 총 준비 시간이 즉시 바뀌며, 보이는 값을 눌러도 선택된다. TalkBack에서는 하나의 조절 컨트롤로 읽히고 `1분 늘리기`/`1분 줄이기` 동작이 있다. 360×800에서 잘리지 않는다.
- Implement: `src/components/wheel-picker.tsx`에 `MinuteWheel`(가로 ScrollView, 52 px 항목 × 3열, 1~120분, snapToInterval, settle 타이머, adjustable 접근성) 추가. `src/app/create.tsx`의 준비 행동 카드를 2행(아이콘·이름·삭제 / `준비 시간` 라벨과 드럼)으로 바꾸고 `setMinutes(id, minutes)`로 절대값 저장(`minutesEditedByUser` 유지). `docs/HARNESS.md` 수동 스모크 51.
...
- 2026-08-27 TMAP 비용/한도 후속: `TimeAgent Premium` 앱에 TMAP 대중교통 Premium(대중교통 0.88원/건, 요약정보 0.55원/건, VAT 포함 후불 자동결제)을 신청했다. 새 키로 `/transit/routes`와 `/transit/routes/sub`를 각각 실호출해 모두 HTTP 200을 확인했다. 새 클라이언트는 `transitSummaryOnly: true`로 구간 없는 저비용 `/sub`를 요청하고 지도 또는 출발 30분 이내 첫 탑승·TAGO 확인처럼 구간이 필요한 때만 상세 `/transit/routes`를 사용한다. 기존 설치본은 이 플래그가 없으므로 상세 응답과 첫 탑승을 유지해 배포 중 기능 회귀를 막는다. 새 키 배포·운영 실호출 확인 전 기존 Free 상품은 해지하지 않는다.
...
- Observe: 10:25~10:28 KST Samsung Android 12 `SM-N971N`에 2-ABI APK를 `adb install -r`로 덮어 설치했다(같은 업로드 키, 데이터 유지: 기존 일정 3건 보존). 새 약속 `서면 볼링장 11:00 · 대중교통`을 만들자 계획 화면 `이동 시간 근거`가 `지하철 35분 · 1,600원 · 환승 1회 · 도보 7분 / TMAP 시간표 기준 · 10:26 계산 · 10:30 출발 기준`으로 뜨고, 출발이 30분 이내라 첫 탑승 `부산2호선 · 해운대`가 상세 조회로 채워졌으며 지하철은 `실시간 도착은 아직 지원하지 않아 시간표 기준` 안내가 나왔다. 서버 로그 순서는 `POST /v1/routes/estimates` 200(10:26:32, 요약) → `POST /v1/routes/transit` 200(10:27:46, 상세) → `POST /v1/arrivals` 200(10:27:46)이다. `지도 보기`에 `TMAP 실제 경로 · 14km · 약 36분` 경로선이 그려졌다. logcat에 FATAL·ReactNativeJS 오류 0건. 계획은 저장하지 않고 닫아 홈이 기존 3건 그대로임을 확인했고 `stay_on_while_plugged_in`은 0으로 되돌렸다. 버스 첫 탑승 + TAGO `실시간` 표시는 TMAP이 지하철을 최선으로 골라 이번 경로에서는 나오지 않았다 — 버스가 최선인 목적지로 별도 관찰이 남는다. 1.0.10 Play 검토가 끝난 뒤 업로드한다.
...
## 2026-08-27 지도·이동수단 카드·준비 시간 드럼·계획 화면 버튼 UI 다듬기 (완료)

- Accept: 계획 화면의 지도는 바깥 카드 없이 지도 상자만 보이고 +/- 버튼 대신 핀치로 확대하며, 지도 위 원형 버튼으로 전체 화면을 연다. 모든 지도의 내 위치 버튼은 원형으로 통일된다. 이동수단 카드는 한 줄에 4개이고 위아래 여백이 넓다. 준비 시간 드럼은 행 전체 폭을 쓰고 연속 조작해도 멈추지 않는다. `N분 여유` 배지의 좌우 여백이 같고, 근거 카드의 외부 지도 버튼은 네이버 지도 하나만, 하단 `준비 시간 수정`·`플랜 B 보기`는 폭을 반반 쓴다.
- Implement: `destination-map.native.tsx`에서 `isShowZoomControls`·네이티브 위치 버튼을 끄고 `onExpand`(원형 전체보기)와 `setLocationTrackingMode('Follow')`를 부르는 원형 내 위치 버튼(`locate` 아이콘 추가)을 지도 위에 겹쳤다. `destination-picker.tsx`는 힌트 줄의 전체보기 버튼을 지도 위 버튼으로 옮겼고, `plan.tsx`는 지도 카드를 없애고 `DestinationMap`만 두며 전체 화면 `Modal`(제목·주소·원형 닫기)을 추가했다. `create.tsx` 이동수단 카드는 `flex: 1`로 4개 한 줄, `paddingVertical: space.xxl`. `MinuteWheel`은 원인이던 매 렌더 `contentOffset` 재적용(Android에서 드래그 중 프로그램 스크롤이 제스처를 취소해 먹통)과 120개 `Pressable` 전체 재렌더를 없앴다 — `onLayout` 폭으로 밴드를 가운데 두고, 항목을 `memo`로 감싸 강조 단계(0·1·2)만 바뀌는 항목만 다시 그리며, 드래그 중에는 외부 값 동기화를 미루고 `disableIntervalMomentum`·`overScrollMode="never"`로 스냅을 안정시켰다. `arrivalStatus`는 `minWidth` 대신 좌우 `paddingHorizontal: 8`·가운데 정렬, `externalMapLinks`는 네이버만 돌려주고(카카오 링크 함수는 유지) 테스트를 맞췄다. 하단 두 버튼은 `flex: 1` 래퍼로 반반.
- Verify: `npm run verify` 56스위트·576/576, ESLint 오류 0(경고 8 기존). arm64 release APK(`ced50927…`, 772 tasks `BUILD SUCCESSFUL`)를 Samsung Android 12 `SM-N971N`에 덮어 설치해 11:06~11:09 KST 화면 녹화(`tmp/timeagent-ui-demo-2026-08-27.mp4`, 14.1 MB)와 함께 확인했다: 지도 상자만 표시·+/- 없음·원형 전체보기 → 전체 화면(원형 닫기) → 닫기, `10분 여유` 배지, 근거 카드에 네이버 버튼만, 하단 `준비 시간 수정`/`플랜 B 보기` 반반, 준비 시간 드럼 5개 값 폭 · 좌우 스와이프 4회 연속(5→11분)과 탭(→12분) 모두 즉시 반영·총 준비 시간 갱신, 이동수단 카드 4개 한 줄, 목적지 지도 원형 전체보기·내 위치 버튼(누르면 현 위치로 이동). logcat FATAL·ReactNativeJS 오류 0건. 편집은 저장하지 않고 닫았다.
...
- Observe: 14:38 KST Samsung Android 12 `SM-N971N`(1.0.12 UI 빌드)의 새 일정 화면에서 장소명 `Starbucks`를 키보드 검색키로 제출하자 `10개의 장소를 찾았습니다`와 서면 인근 스타벅스 목록(서면중앙대로R점·부산서면점 등, 주소 포함)이 표시됐다. 서버 로그 `GET /v1/places?query=Starbucks&latitude=35.1531&longitude=129.0597` 200(14:38:23). 검색 버튼 탭 경로와 결과 선택은 두 번째 시도 중 폰이 잠겨 이번에는 확인하지 못했다. 앞서 SM-S931N(Play 1.0.9)에서는 검색 버튼 탭이 서버 요청을 남기지 않았는데(로그에 `/v1/places` 없음), adb 탭이 키보드 위 좌표로 갔을 가능성이 높아 재확인 대상이다. 임시 함수는 `npx supabase functions delete permission-probe --project-ref chpsoncuxjpgugowrydb`로 지운다. TMAP 키를 앱별로 나눌 때는 `TMAP_BASIC_APP_KEY`를 등록하고 mobility를 재배포하면 코드가 바로 반영한다.

...
## 2026-08-13 무발화 자동 제출과 가짜 일정 생성 차단 (실기기 재검증 대기)

- Observe: 실기기 `SM-N971N`에서 음성 화면을 열고 아무 말도 하지 않았는데 약 12초 뒤 녹음이 전송됐고, AI가 `다음 주 금요일 오후 2시에 강남역에서 약속 잡고 싶어`라는 사용자 발화를 지어내 일정 등록을 진행했다. 48초 관찰에서 `듣는 중`→`정리 중`→가짜 발화 버블 1개가 재현됐다. 화면을 켜두기만 해도 없는 약속이 만들어질 수 있는 상태였다.
- Cause: 발화 판정이 고정 -55dB 절대 기준에 의존해, 이 기준보다 시끄러운 방에서는 주변 소음이 계속 발화로 인식됐다. 소음이 잠깐 잦아들면 무음 녹음이 그대로 전송됐다. 서버의 Gemini는 무음 오디오에 대해 빈 전사문을 돌려주지 않고 문장을 지어냈다.
- Implement: 측정값이 일정 개수 쌓여 소음 바닥을 신뢰할 수 있게 되면 고정 기준을 더 이상 쓰지 않고, 발화는 측정된 소음 바닥보다 뚜렷하게 큰 소리여야 인정하도록 바꿨다. 고정 기준만으로 인정된 발화도 최대 음량이 소음 바닥보다 충분히 크지 않으면 철회한다. 운영 Gemini 지침에 알아들을 수 있는 사람의 말이 없으면 transcript를 비우고 지어내지 말라는 규칙을 추가했다.
...
- Evidence: `src/lib/voice-schedule-assistant.ts`, `src/lib/__tests__/voice-schedule-assistant.test.ts`, `supabase/functions/_shared/gemini-assistant.ts`.
- 남은 작업: 수정본 APK를 설치해 같은 무발화 조건에서 제출과 가짜 발화가 사라졌는지 확인하고, 실제 발화가 정상 인식되는지 확인한다. 운영 `assistant` Edge Function 재배포도 필요하다.

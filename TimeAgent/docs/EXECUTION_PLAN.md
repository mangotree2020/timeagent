...
- Limit: 세 사례는 그대로 남았다. `서면역→역삼역`, `부산역→구산역`, `초읍→초원`은 발음 규칙으로 설명되지 않아 원래 이름을 만들어 낼 수 없다. 앞 둘은 기존 거리·이름 검증이 이미 자동 확정을 막고 있어 사용자가 목록에서 고른다. 실기기 검증은 미실시이며 다음 실기기 회차에서 확인해야 한다.

...
- [x] 세 기준 화면 크기 스크린샷 회귀
  ...
  - `npm run visual:test` 시각·키보드·Journey 핵심 순서 42/42 통과, 대표 360/430px 화면 육안 검토 완료.
- [x] 폰트 200%, 스크린리더, 키보드 접근성 점검
  ...
  - Android 12 `SM-N971N`에서 TalkBack·터치 탐색을 활성화하고 Journey의 헤더·현재 상태·오류 해결·다음 행동·주 CTA·지도 순서와 실제 포커스 링을 확인했다.
  - 다음 행동 상태 갱신 시 Google TTS의 접근성 음성 스트림 재생을 확인했으며, 앱 자체 음성 버튼은 `화면 읽기 사용 중, 앱 음성 자동 재생 중지됨` 상태로 비활성화되어 중복 발화하지 않았다.
  - Journey 핵심 정보와 주 CTA가 지도보다 먼저 배치되는 자동 수용 기준을 추가하고 세 viewport에서 통과했다.
...
## 2026-07-27 Ralph Loop - TalkBack 핵심 탐색 순서 실기기 검증 (완료)

- Observe: TalkBack 활성 Journey에서 지도와 백그라운드 보조 기능이 다음 행동·주 CTA보다 앞에 놓여 핵심 탐색 순서를 보장하지 못했고, 기본 Node 16 환경은 Expo 검증을 시작하지 못했음.
- Select: 사용자가 이동 중 현재 상태를 확인한 뒤 다음 행동과 `목적지 도착 확인`에 지도보다 먼저 도달하는 결과를 선택.
- Specify: 현재 상태→다음 행동→주 CTA의 화면·탐색 순서를 유지하고, 지도·백그라운드 기능은 그 뒤에 제공해야 함. TalkBack 활성 중 앱 TTS는 중복 발화하지 않고 상태 변화는 접근성 음성으로 전달해야 함.
- Implement: Journey의 다음 행동과 주 CTA를 지도 앞에 재배치하고, 세 viewport에서 핵심 요소의 실제 위치 순서를 검증하는 Playwright 수용 기준을 추가.
- Verify: Node 24.18에서 `npm run verify` TypeScript/ESLint/Jest 65/65, Expo Doctor 20/20, `npm run visual:test` 42/42 통과. Android 12 `SM-N971N`에서 TalkBack·터치 탐색 활성, 핵심 요소 접근성 트리 순서, 실제 포커스 링, 접근성 TTS 스트림, 앱 음성 비활성 상태를 확인.
- Reflect: 실험적 접근성 순서 API는 누락 요소를 탐색 불가능하게 만들 수 있어 사용하지 않고, 시각 순서와 접근성 순서를 일치시켜 핵심 행동의 우선순위를 개선함.
- Evidence: `src/app/journey.tsx`, `e2e/visual/app.visual.spec.mjs`, `e2e/visual/__screenshots__/*/journey-*.png`, `/tmp/timeagent-talkback-after.xml`, `/tmp/talkback-dpad-*.png`.

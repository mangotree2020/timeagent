...
운영 proxy는 Supabase Edge Function `mobility`이며 기준 URL은 `https://chpsoncuxjpgugowrydb.supabase.co/functions/v1/mobility`이다. API endpoint는 이 주소 아래의 `/v1/places`, `/v1/geocode`, `/v1/reverse-geocode`, `/v1/routes/walk`, `/v1/routes/estimates`(약속 출발 시각 `departureAt` 기준 요약, 60초 캐시), `/v1/routes/transit`(구간·정류장·경로선 상세), `/v1/arrivals`(선택 경로 첫 탑승 정류장의 TAGO 실시간 도착, 20초 캐시) 계약으로 제공한다. 새 클라이언트는 `/v1/routes/estimates`에 `transitSummaryOnly: true`를 보내 평소 계획 계산에 0.55원/건인 `/transit/routes/sub`를 사용하고, 지도 또는 출발 30분 이내 첫 탑승 확인처럼 구간이 필요한 때에만 0.88원/건인 `/transit/routes`를 사용한다. 플래그가 없는 기존 설치본은 첫 탑승 정보가 사라지지 않도록 상세 API를 유지한다. `/health`는 공급자별 호출 수·성공률·지연시간을 원문 위치·검색어 없이 집계해 돌려준다. Supabase 기본 도메인과 관리형 HTTPS를 사용하므로 `timeagent.mangonw.com` DNS·인증서 설정은 현재 범위에서 필요하지 않다.

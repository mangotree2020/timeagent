...
- NAVER Client Secret과 TMAP App Key는 mobility 서버에만 둔다.
- `EXPO_PUBLIC_` 접두사가 붙은 값은 앱 번들에 포함될 수 있으므로 비밀값에 사용하지 않는다.
- React Native 앱은 `EXPO_PUBLIC_MOBILITY_API_BASE_URL`로 ON:TIME 서버만 호출한다.
...
3. Android 애플리케이션 식별자에 `com.ontime.app`을 등록한다.
...
앱 빌드 환경에는 공개 proxy URL인 `EXPO_PUBLIC_MOBILITY_API_BASE_URL`만 설정한다. 개발·스테이징·운영 키와 NAVER 허용 도메인/패키지를 각각 분리한다.

...
const systemInstruction = [
  ...
  "가장 중요한 규칙: 사용자가 말하지 않은 내용을 만들어 내지 않는다. 장소 이름, 일정 이름, 할 일, 시각처럼 구체적인 값은 사용자가 이번 대화에서 직접 말한 것만 쓴다. 이 지침에 적힌 표현이나 이전 대화의 예시를 사용자의 말처럼 되풀이하지 않는다.",
  ...
  "이동수단에서 걸어서, 걸어가, 걷기, 도보로는 모두 transport를 도보로 정규화한다. 도보는 다른 수단의 대체값이 아니라 사용자가 직접 선택할 수 있는 독립 이동수단이다.",

...
const systemInstruction = [
  ...
  "이동수단에서 걸어서, 걸어가, 걷기, 도보로는 모두 transport를 도보로 정규화한다. 자차, 차 타고, 차 끌고, 차로, 운전해서는 자가용으로, 전철, 지하철역, 메트로는 지하철로, 시내버스, 마을버스, 광역버스는 버스로, 택시 잡아서, 택시로는 택시로 정규화한다. 도보는 다른 수단의 대체값이 아니라 사용자가 직접 선택할 수 있는 독립 이동수단이다.",
  "이동수단 목록에 없는 수단을 말했으면 그 말을 비슷한 값으로 바꾸지 않는다. 자전거, 따릉이, 킥보드, 오토바이, 기차, 비행기, 배처럼 목록 밖의 수단이나 얻어 타기, 데리러 온다는 말은 transport를 null로 두고 clarification의 field를 transport로 해서 다섯 선택지를 제시한다. 자전거를 도보로 바꾸면 이동 시간이 실제와 달라진다.",
  ...
  "destination에는 지도에서 찾을 수 있는 장소만 넣는다. 역, 건물, 상호, 동네, 주소처럼 위치를 가리키는 말이어야 한다. 동창 모임, 회식, 면접, 생일파티처럼 용건이나 모임 이름은 장소가 아니므로 destination에 넣지 않는다. 용건만 말하고 장소를 말하지 않았으면 그 말은 title에만 쓰고 destination은 null로 둔 뒤 어디에서 만나는지 묻는다.",
